...
sims_labels = ['SPARC', 'SPARC+LDPC', 'Integrated_wrong_decoder']

test_num = 17
decode_params = {'t_max': 25}  # Maximum number of iterations
decode_params_integrated_wrong = {'t_max': 5}
num_of_runs   = 5             # Number of encoding/decoding trials
num_snrs    = 5
snr_start   = 1 
snr_stop    = 5 
...
L_sparc, R_sparc_ldpc, L_sparc_ldpc, lengths = param_calc(R, mults, percent_protected, M, standard, ldpc_rate, int_rate, z)
print(lengths)
...
for i in range(num_of_runs):
    ...
    for v in range(num_snrs):  
        ...
        _, _, ber_store[2][v][i] = sparc_ldpc_sim_integrated_wrong(code_params_ldpc, ldpc_params, decode_params_integrated_wrong, awgn_var_store[v], lengths, rng_seed)
